...
local running = false
local parts = {} -- tabela pra guardar todos os parts criados

-- Toggle
local Toggle1 = Tab1:AddToggle({
    Name = "spawnar cachorro",
...
Toggle1:Callback(function(Value)
    ...
    if not running then
        -- Quando desligar, remove todos os parts criados
...
spawn(function()
    while true do
        if running then
            ...
            dued.Anchored = false
            dued.Position = Vector3.new(math.random(-20,20), math.random(5,15), math.random(-20,20))

            table.insert(parts, dued) -- guarda na tabela

            -- Movimento aleatório do part em spawn separado
            spawn(function()
                while running and dued.Parent do
        ...
        wait(0.5) -- cria um novo part a cada meio segundo
    end
end)